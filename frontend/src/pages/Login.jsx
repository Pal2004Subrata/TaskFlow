...
const Login = () => {
  ...
  const onSubmitForm = async (data) => {
    try {
      ...
      setError('');
      setFormData(data);
      await api.post('/auth/send-login-otp', data);
      setStep(2);
    } catch (err) {
  ...
  const handleResendOtp = async () => {
    setError('');
    await api.post('/auth/send-login-otp', formData);
  };
    ...
    <div className="relative min-h-screen flex bg-[#F8FAFC] dark:bg-[#020617] font-sans transition-colors overflow-hidden">
      ...
      <div className="hidden lg:flex lg:w-1/2 text-slate-900 dark:text-white p-12 flex-col justify-between relative overflow-hidden border-r border-slate-200/50 dark:border-slate-800/50 transition-colors">
        ...
        <div className="relative z-10 max-w-md mx-auto mt-16 text-center">
           ...
           >
             <h2 className="text-4xl lg:text-5xl font-extrabold mb-6 leading-[1.1] tracking-tight text-slate-900 dark:text-white">Start managing your tasks beautifully.</h2>
             ...
             <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none">
                ...
                <p className="text-slate-700 dark:text-slate-300 mb-6 font-medium leading-relaxed">"TaskFlow has completely transformed how our engineering team operates. It's incredibly fast, intuitive, and the design is unmatched."</p>
                <div className="flex items-center gap-3 justify-center text-left">
                   ...
                   <div>
                      <div className="font-semibold text-sm text-slate-900 dark:text-white">Sarah Jenkins</div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 relative z-10 bg-white/40 dark:bg-[#020617]/40 backdrop-blur-xl transition-colors">
         <div className="absolute top-8 left-8 right-8 lg:left-12 lg:right-12 flex justify-between items-center z-20">
            {step === 2 ? (
              ...
              >
                <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" /> Re-enter Credentials
              </button>
            ) : (
            ...
            {step === 1 ? (
              <>
                ...
                <p className="mt-8 text-center lg:text-left text-sm text-slate-500 dark:text-slate-400 font-medium">
                   Don't have an account? <Link to="/signup" className="text-indigo-600 dark:text-indigo-400 font-semibold hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors">Sign up for free</Link>
                </p>
              </>
            ) : (
              <OtpInput
                ...
                description="Enter the 6-digit login security code sent to"
              />
            )}
         </motion.div>
      </div>
